...
def graphColourUtil(assignment, grp, v):
    global nodesGen
    if v == len(grp):
        return True
    ...
    for c in Nodes[node - 1].domain:
        ...
        if isSafe(v, grp, assignment, c):
            ...
            assignment[v] = c
            if graphColourUtil(assignment, grp, v + 1):
                return True
            # print("Clearing Assignment {} ".format(v))
            assignment[v] = 0


def graphColouring(i):
    size = len(Group[i])
    assignment = [0] * size
    if graphColourUtil(assignment, Group[i], 0) is None:
